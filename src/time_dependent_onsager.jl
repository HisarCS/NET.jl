...
"""
    compute_flux_single_time(L_t::Array{Float64, 2}, F_t::Array{Float64, 1}) -> Array{Float64, 1}

Computes the flux vector for a single time point using manual matrix-vector multiplication.
- L_t: 2D Onsager matrix at a specific time point.
- F_t: 1D force vector at the same time point.
Returns: A 1D array of computed fluxes for the time point.
"""
function compute_flux_single_time(L_t::Array{Float64, 2}, F_t::Array{Float64, 1})
    num_vars = size(L_t, 1)
    J_t = zeros(num_vars)
    for i in 1:num_vars
        J_t[i] = sum(L_t[i, j] * F_t[j] for j in 1:num_vars)
    end
    return J_t
end
...
function compute_time_dependent_fluxes(L::TimeDependentOnsagerMatrix, F::Array{Float64, 2})
    ...
    for t in 1:num_times
        J[:, t] = compute_flux_single_time(L.L[:, :, t], F[:, t])
    end
    info("Time-dependent flux computation complete for $num_times time points.")
    return J
end
