...
function compute_local_entropy_production(J::Array{Float64, 2}, X::Array{Float64, 2})
    validate_dimensions_entropy_le(J, X)
    num_points = size(J, 2)
    entropy_production = zeros(num_points)
    for k in 1:num_points
        entropy_production[k] = sum(J[:, k] .* X[:, k])
    end
    info("Local entropy production calculation complete for $num_points grid points.")
...
"""
function validate_dimensions_entropy_le(J::Array{Float64, 2}, X::Array{Float64, 2})
    if size(J) != size(X)
